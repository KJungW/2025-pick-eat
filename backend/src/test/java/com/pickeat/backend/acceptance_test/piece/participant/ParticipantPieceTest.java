...
public class ParticipantPieceTest {
    ...
    public static List<ParticipantResponse> 모든_참가자_메타데이터_조회(String participantToken) {
        return RestAssured
                .given().log().all()
                .header("Pickeat-Participant-Token", "Bearer " + participantToken)
                .when()
                .get("/api/v1/participants/meta")
                .then().log().all()
                .statusCode(HttpStatus.OK.value())
                .extract()
                .as(new TypeRef<>() {
    ...
    public static ParticipantStateResponse 모든_참가자_상태_조회(String participantToken) {
        return RestAssured
                .given().log().all()
                .header("Pickeat-Participant-Token", "Bearer " + participantToken)
                .when()
                .get("/api/v1/participants/state")
                .then().log().all()
                .statusCode(HttpStatus.OK.value())
                .extract()
